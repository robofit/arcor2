...
def test_sphere_up(start_processes: Urls) -> None:
    ...
    scene_service.upsert_graspable(
        cyl, Pose(Position(0.5, 0.5, 0.5), Orientation(0, 0, 0, 1)), state="WORLD", source="test"
    )
    assert cyl.id in scene_service.graspable_ids()
    time.sleep(1)
    ...
    cyl = Cylinder("Cyl3", 0.1, 0.95)
    scene_service.upsert_graspable(
        cyl, Pose(Position(0.0, 0.0, 0.0), Orientation(0, 0, 0, 1)), state="ATTACHED", source="test"
    )
    assert cyl.id in scene_service.graspable_ids()
    time.sleep(1)
