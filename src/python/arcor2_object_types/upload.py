...
from arcor2_object_types.abstract import CollisionObject, Generic, Robot
from arcor2_object_types.utils import check_object_type, get_containing_module_sources
...
def upload_whatever(type_def: type[object]) -> None:
    ...
    client.update_object_type(obj_type)
...
def upload_def(
    ...
    """Uploads ObjectType definition to the Storage service."""
        ...
        if isinstance(model, Mesh):
            ...
            try:
                with open(file_to_upload, "rb") as f:
                    client.create_asset(type_def.mesh_filename, f.read(), description=f"mesh for {obj_type.id}")
            except OSError as e:
                raise UploadException(f"Failed to read mesh file. {str(e)}")

        client.put_model(model)
    ...
    if urdf:
        ...
        mem_zip.seek(0)
        client.create_asset(urdf.archive_name, mem_zip.getvalue(), description=f"URDF for {obj_type.id}")

    if obj_type.id not in {oti.id for oti in client.get_object_type_ids()}:
        for ai in client.asset_info():
            if ai.file_name == f"{obj_type.id}.py":
                client.delete_asset(ai.id)
